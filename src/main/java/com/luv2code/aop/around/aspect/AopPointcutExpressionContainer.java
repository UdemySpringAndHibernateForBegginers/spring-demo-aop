...
public class AopPointcutExpressionContainer {
    ...
    @Pointcut("execution(* com.luv2code.aop.around.dao.*.*(..))") //uruchomi się dla wszystkich metod z zadanego pakietu, bez względu na nazwę i parametry
    public void forDaoPackage() {}

    @Pointcut("execution(* com.luv2code.aop.around.dao.*.get*(..))") //uruchomi się dla wszystkich getterów z zadanego pakietu
    public void forGetters() {}

    @Pointcut("execution(* com.luv2code.aop.around.dao.*.set*(..))") //uruchomi się dla wszystkich setterów z zadanego pakietu
